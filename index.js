...
class Quiz {
    ...
    get averageTime() {
        if (this.questionTimes.length < 2) {
            return null;
        }
        let sum = 0;
    ...
    incrementCurrentQuestionAttempts() {
    ...
    check(guess) {
        if (this.currentQuestion >= this.piece.notes.length) {
            return false;
        }
...
$(document).keydown(function (event) {
    switch (event.key) {
        case "c":
            check(new PitchClass("C"));
            break;
        case "d":
            check(new PitchClass("D"));
            break;
        case "e":
            check(new PitchClass("E"));
            break;
        case "f":
            check(new PitchClass("F"));
            break;
        case "g":
            check(new PitchClass("G"));
            break;
        case "a":
            check(new PitchClass("A"));
            break;
        case "b":
            check(new PitchClass("B"));
            break;
        case "Enter":
            if (quiz.isFinished) {
                restartQuiz();
